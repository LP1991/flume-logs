...
import static com.freitas.flume.serializer.TaildirSourceConfigurationConstants.BYTE_OFFSET_HEADER_KEY;
...
public class TailFile {
    ...
    private boolean flag_line = false;
    ...
    public LineResult readLine() throws IOException {
        ...
        while (true) {
            if (bufferPos == NEED_READING) {
                if (raf.getFilePointer() < raf.length()) {
                    ...
                    if (oldBuffer.length > 0) {
                        lineResult = new LineResult(true, oldBuffer);
                        oldBuffer = new byte[0];
                        logger.info("lineResult.line.length ===========" +lineResult.line.length);
                        setLineReadPos(lineReadPos + lineResult.line.length);
                        flag_line = false;
                        logger.info("at file end  !");
                    }
                    break;
                }
            }
            for (int i = bufferPos; i < buffer.length; i++) {
                if(flag_line){
                    logger.info("enter flag_line  !");
                    if(buffer[i]>48 && buffer[i]<=57){
                        logger.info("buffer[i]>48 && buffer[i]<=57");
                        int oldLen = oldBuffer.length;
                        // Don't copy last byte(NEW_LINE)
                        int lineLen = i - bufferPos;
                        lineLen = lineLen==0?0:lineLen-1;
                        ...
                        logger.info("(oldBuffer.length + lineLen) ===========" +(oldBuffer.length + lineLen));
                        setLineReadPos(lineReadPos + (oldBuffer.length + lineLen));
                        oldBuffer = new byte[]{buffer[i]};
                        ...
                        logger.info("buffer[i]>48 <> buffer[i]<=57");
//                        setLineReadPos(lineReadPos + (oldBuffer.length + (i - bufferPos)));
                        flag_line = false;
                    }
                }
...
//                    break;
                    ...
                    logger.info("buffer[i] == BYTE_NL");

                }

            }
            if (lineResult != null) {
                logger.info("ineResult != null");
                break;
            }
            logger.info("ineResult == null");
