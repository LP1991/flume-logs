/********************** 版权声明 *************************
 * 文件名: ReliableTaildirEventReader.java
    ...
    public List<Long> updateTailFiles(boolean skipToEnd) throws IOException {
        ...
        for (TaildirMatcher taildir : taildirCache) {
            ...
            for (File f : taildir.getMatchingFiles()) {
                ...
                TailFile tf = tailFiles.get(inode);
                logger.info("------------tf == null  "+(tf == null));
                logger.info("------------!tf.getPath().equals(f.getAbsolutePath())  "+(!tf.getPath().equals(f.getAbsolutePath())));
                if (tf == null || !tf.getPath().equals(f.getAbsolutePath())) {
                    ...
                    if (updated) {
                        if (tf.getRaf() == null) {
                            logger.info("------------tf.getPos()"+tf.getPos());
                            tf = openFile(f, headers, inode, tf.getPos());
                        }
                        if (f.length() < tf.getPos()) {
                            logger.info("Pos ="+ tf.getPos() +"   ----        length = "+f.length());
